...
原始 L-SHADE 在初期下降后较早进入平台期，约在 5000 次 NFE 后目标值变化趋于平缓，最终仅收敛至接近 $-0.02$ 的水平。这表明原始 L-SHADE 虽具备一定全局搜索能力，但在异构 UAV 动态路由规划问题中容易出现早熟收敛。该问题同时包含 UAV 运动控制、覆盖关系、通信速率、队列演化和能量消耗等多个耦合因素，搜索空间具有高维、非线性和强时序耦合特征。原始 L-SHADE 未显式利用覆盖结构和网络状态信息，因此难以持续引导种群向高覆盖、高吞吐且低违约的区域演化。

相比之下，CFI-L-SHADE 在优化初期即获得更低的目标函数值，并在多数迭代阶段明显优于原始 L-SHADE。这说明覆盖优先初始化能够有效改善初始种群质量，使算法在搜索开始时便具备较好的覆盖结构，从而减少从随机解中构造有效覆盖方案的搜索成本。然而，CFI-L-SHADE 在中后期同样逐渐趋于平缓，表明该方法主要改善算法起点，而对后续搜索过程中覆盖状态的持续保持和调整能力仍然有限。

CSA-L-SHADE 在前期并非始终优于 CFI-L-SHADE，但在中后期仍保持较明显的下降趋势，并最终取得最低的 $J_"total"$。该现象表明，覆盖状态感知机制不仅能够继承覆盖优先初始化带来的结构优势，而且能够在后续迭代过程中持续引导搜索方向，使种群在覆盖保持、吞吐提升和资源消耗控制之间形成更优折中。因此，从收敛曲线可以看出，CSA-L-SHADE 的主要优势并非单纯体现在早期收敛速度，而是体现在中后期持续开发能力和最终解质量上。需要注意的是，CSA-L-SHADE 在后期阴影区间相对较宽，说明不同独立运行之间存在一定波动。因此，本文将其结论表述为最终综合性能更优、后期优化能力更强，而不简单概括为全过程稳定性最强。
...
从最终覆盖率看，原始 L-SHADE 的覆盖率为 $0.8274$，CFI-L-SHADE 提升至 $0.8596$，CSA-L-SHADE 进一步提升至 $0.8943$。相较于原始 L-SHADE，CSA-L-SHADE 的覆盖率提升 $0.0669$，即提升 6.69 个百分点；相较于 CFI-L-SHADE，仍提升 $0.0347$，即提升 3.47 个百分点。这说明覆盖优先初始化能够明显改善 UAV 对任务节点的初始服务结构，而 CSA-L-SHADE 在此基础上进一步增强了搜索过程中对覆盖状态的保持能力。

从覆盖违约度看，原始 L-SHADE 的 $V_"cov"^"norm"$ 为 $0.1726$，CFI-L-SHADE 降至 $0.1505$，CSA-L-SHADE 进一步降至 $0.1076$。与原始 L-SHADE 相比，CSA-L-SHADE 将覆盖违约度降低约 37.7%；与 CFI-L-SHADE 相比，降低约 28.5%。覆盖违约度下降说明 CSA-L-SHADE 不仅提高了平均覆盖比例，而且有效减少了任务周期内长期未被服务的任务节点数量。

该结果表明，所提算法能够较好适应 UAV 通信网络中覆盖关系的离散切换特征。在动态路由规划问题中，UAV 的小幅位置变化可能导致任务节点由可覆盖状态转变为不可覆盖状态，因此覆盖性能对搜索方向较为敏感。CSA-L-SHADE 通过引入覆盖状态感知引导，使种群在变异和选择过程中更倾向于保留具有较好覆盖结构的个体，从而降低有效覆盖结构被破坏的概率。当前固定环境下三种算法的功能性连通违约均接近 0，说明本组实验的主要差异集中在覆盖能力、吞吐表现、队列负载和能耗水平上。
...
在吞吐量方面，原始 L-SHADE 的 $R_"norm"$ 为 $0.2826$，CFI-L-SHADE 提升至 $0.4368$，CSA-L-SHADE 进一步提升至 $0.4641$。与原始 L-SHADE 相比，CSA-L-SHADE 的吞吐量提升约 64.2%。这一结果说明，覆盖性能提升能够有效转化为通信收益。原始 L-SHADE 虽能获得一定可行解，但由于覆盖结构和通信回传结构不充分，导致系统有效吞吐受限。CFI-L-SHADE 通过改善初始覆盖结构提高了数据采集和回传能力，而 CSA-L-SHADE 进一步利用状态感知机制强化了对高价值通信结构的搜索，因此获得最高吞吐量。

在队列负载方面，原始 L-SHADE 的 $Q_"norm"$ 为 $0.3533$，CFI-L-SHADE 降至 $0.2918$，CSA-L-SHADE 进一步降至 $0.2839$。CSA-L-SHADE 相比原始 L-SHADE 降低约 19.6%，说明其能够有效缓解数据积压问题。由于 UAV 队列长度受到任务节点数据到达量和 Sink 回传能力共同影响，较低的队列负载表明算法不仅提高了覆盖能力，也在一定程度上改善了数据转发和链路利用效率。不过，CSA-L-SHADE 与 CFI-L-SHADE 在队列负载上的差距较小，且误差区间存在重叠，因此该指标上的优势不如覆盖率和吞吐量显著。

在能耗方面，原始 L-SHADE 的 $E_"norm"$ 为 $0.000416$，CFI-L-SHADE 为 $0.000407$，CSA-L-SHADE 进一步降至 $0.000391$。虽然三者绝对数值差异不大，但整体趋势保持一致，即 CSA-L-SHADE 在获得更高覆盖率和更高吞吐量的同时，并未带来额外能耗开销，反而取得最低能耗均值。这说明所提算法不是通过过度移动或提高通信代价来换取性能提升，而是通过更合理的 UAV 轨迹、覆盖关系和通信结构减少了无效运动与低效传输。

综合来看，CSA-L-SHADE 在覆盖率、覆盖违约度、吞吐量、队列负载和能耗五个指标上均取得最优均值，表明其能够在服务质量和资源消耗之间形成较优折中。尤其是吞吐量显著提升而能耗保持较低，说明算法在复杂约束下具备较好的能效表现。

=== 不同任务节点规模下的性能分析

为进一步考察算法对任务负载变化的适应能力，本文改变地面任务节点数量，并比较三种算法在覆盖率、能耗、队列负载和吞吐量方面的变化趋势。该实验关注任务节点规模变化对算法性能的影响，因此采用三种算法对比折线图，而不是单独展示 CSA-L-SHADE 的参数敏感性。
...
从图 5-4 可以看出，三种算法的覆盖率随任务节点数量变化呈现非单调波动。这说明系统覆盖性能不仅取决于任务节点数量，还受到任务节点空间分布、UAV 覆盖半径、通信半径以及 Sink 位置等因素共同影响。在多数任务节点规模下，CSA-L-SHADE 均取得最高覆盖率，特别是在任务节点数量较多时仍能保持相对较好的覆盖水平，表明其对高负载场景具有更强适应性。相比之下，CFI-L-SHADE 虽在中等规模场景下表现较好，但在部分高任务节点数场景中覆盖率下降较明显，说明仅依赖覆盖优先初始化难以完全应对复杂任务分布下的后续搜索需求。
...
从图 5-5 可以看出，原始 L-SHADE 在多数场景下能耗较高，且波动幅度较大，说明其对任务节点分布和场景约束变化较为敏感。CFI-L-SHADE 和 CSA-L-SHADE 的能耗整体低于原始 L-SHADE，表明覆盖结构引导能够减少无效飞行和低效通信带来的能量浪费。CSA-L-SHADE 在部分任务规模下并非能耗最低，但其通常同时获得更高覆盖率和更高吞吐量，因此体现出更偏向综合性能优化的特征。
...
从图 5-6 可以看出，队列指标对任务节点数量变化较为敏感。当任务节点数量增加到一定程度时，系统数据到达量显著增加，若 Sink 回传能力或中继链路能力不足，UAV 缓存队列可能快速积压。实验中部分任务节点规模下出现覆盖率和吞吐量较高但队列负载同时升高的现象，说明高覆盖率并不必然带来低队列负载。覆盖能力增强后，系统采集的数据量增加，若回传链路未同步增强，则可能导致队列压力上升。因此，在 UAV 通信网络动态路由规划中，仅优化覆盖率是不充分的，必须同时考虑吞吐量和队列稳定性。
...
从图 5-7 可以看出，CSA-L-SHADE 在中高任务节点规模下表现较为突出，尤其在高负载场景下能够保持较高吞吐水平。这表明覆盖状态感知机制不仅改善任务节点服务范围，而且能够帮助算法搜索更有效的数据回传结构。相比之下，原始 L-SHADE 在多数规模下吞吐量较低，说明缺少问题结构引导时，算法难以充分利用 UAV 的通信资源。

总体而言，任务节点规模实验表明，CSA-L-SHADE 对复杂负载场景具有更强适应性。其优势主要体现在两个方面：一是能够在任务节点数量变化时保持较高覆盖率，二是能够在高负载条件下维持较好的吞吐能力。但队列负载结果也表明，当前算法仍可能受到 Sink 回传瓶颈和数据积压效应的影响，后续可进一步引入队列状态和链路状态反馈，以提升系统长期运行稳定性。

=== 综合讨论

上述实验结果表明，原始 L-SHADE 在本文问题中存在明显局限。虽然其能够在优化初期快速降低目标函数值，但由于缺乏对覆盖结构和通信状态的显式利用，后期容易陷入平台期，最终在覆盖率、吞吐量、队列负载和能耗等指标上均表现较弱。

CFI-L-SHADE 通过覆盖优先初始化显著改善了算法初始解质量，使其在收敛初期和最终覆盖性能上均优于原始 L-SHADE。这说明在异构 UAV 通信网络动态路由规划问题中，覆盖结构是影响算法性能的重要先验信息。然而，CFI-L-SHADE 的改进主要发生在初始化阶段，后续搜索仍可能破坏已形成的有效覆盖结构，因此其最终性能提升存在一定上限。

CSA-L-SHADE 在 CFI-L-SHADE 的基础上进一步引入覆盖状态感知机制，使算法能够在迭代过程中持续关注覆盖结构变化，并在覆盖率、吞吐量、队列负载和能耗之间形成更优平衡。实验结果显示，CSA-L-SHADE 最终覆盖率达到 $0.8943$，覆盖违约度降至 $0.1076$，吞吐量提升至 $0.4641$，队列负载和能耗分别降至 $0.2839$ 和 $0.000391$。上述结果说明，CSA-L-SHADE 能够有效提升系统服务能力，同时避免由性能提升带来的额外资源消耗。

不过，从标准差和收敛曲线阴影范围可以看出，CSA-L-SHADE 在部分指标上存在一定波动，尤其是吞吐量和后期目标函数值的方差相对较大。这说明状态感知机制虽然提升了算法的优化上限，但其对随机种子和场景结构仍具有一定敏感性。后续研究可从两个方向进一步完善：一是增加独立运行次数并进行显著性检验，以增强统计结论可靠性；二是引入更精细的队列状态和链路状态反馈，使算法在提高覆盖和吞吐的同时进一步降低运行波动。

综上，CSA-L-SHADE 在固定环境和不同任务节点规模下均表现出较优的综合性能，验证了覆盖状态感知机制在异构无人机通信网络动态路由规划中的有效性。该算法能够较好地适应覆盖、吞吐、队列和能耗之间的多指标耦合关系，为复杂应急通信场景下 UAV 集群路由规划提供了一种有效的启发式求解方法。
...
目标函数方面，本文构建由归一化吞吐量、队列负载、能耗、覆盖违约和连通性违约组成的综合评价指标。该指标既鼓励提高 Sink 侧数据汇聚能力，又对覆盖不足、队列积压、能量消耗和功能性回传能力不足进行惩罚。进一步地，本文在加权目标函数之外引入覆盖与回传优先的层级判优逻辑，使算法在搜索过程中优先维持基础覆盖和数据回传能力，再进一步优化吞吐量、队列负载和能耗。

算法入口方面，本文引入覆盖优先初始化机制。原始 L-SHADE 随机初始化缺少任务节点空间信息，搜索早期容易出现低覆盖轨迹；覆盖导向初始化方法则根据地面任务节点分布生成一部分初始个体。该机制通过目标点选择、UAV-目标匹配和“接近-保持”式控制序列生成，使初始种群中包含更接近覆盖可行区域的个体。实验结果显示，CFI-L-SHADE 相较原始 L-SHADE 在收敛初期和最终覆盖性能上均有所改善，说明覆盖结构信息能够为后续差分进化搜索提供更合理的搜索入口。

参数控制方面，本文提出覆盖状态感知 CSA-L-SHADE 算法。原始 L-SHADE 参数自适应主要依赖历史成功经验，缺少对 UAV 覆盖状态的显式感知。本文构造覆盖状态指标和有限状态机，将搜索过程划分为覆盖构造、覆盖稳定、性能精修和覆盖恢复四类状态，并根据不同状态动态约束 $p$、$F$ 和 $"CR"$。这样，L-SHADE 可根据种群覆盖状态调整搜索尺度和搜索方向，在保持覆盖结构的同时优化吞吐量、队列负载和能耗水平。

实验验证方面，本文在固定异构环境下对 L-SHADE、CFI-L-SHADE 和 CSA-L-SHADE 进行对比，并进一步考察不同任务节点数量下的性能变化。固定环境实验表明，CSA-L-SHADE 最终覆盖率达到 $0.8943$，覆盖违约度降至 $0.1076$，吞吐量提升至 $0.4641$，队列负载和能耗分别降至 $0.2839$ 和 $0.000391$，在多个指标上取得最优均值。收敛曲线显示，CSA-L-SHADE 的优势主要体现在中后期持续开发能力和最终解质量上，而不是单纯的初期收敛速度。任务节点规模实验进一步表明，CSA-L-SHADE 在多数规模下能够保持较高覆盖率，并在中高负载场景下维持较好的吞吐能力，说明覆盖状态感知机制对复杂负载变化具有一定适应性。

== 不足与展望

当前研究仍有不足，后续工作可以从模型真实性、路由粒度、实验验证和参数自适应等方向继续推进。
...
实验验证仍有进一步扩展空间。本文已在固定环境和不同任务节点规模下验证算法性能，但独立运行次数较少，且未单独进行显著性检验。从收敛曲线阴影范围和部分指标标准差可以看出，CSA-L-SHADE 在提升优化上限的同时仍存在一定随机波动，尤其在吞吐量和后期目标函数值上更为明显。后续应增加独立运行次数，引入显著性检验，并在多随机任务分布、多 UAV 规模和不同数据产生强度下验证算法稳定性。

队列与回传瓶颈仍需进一步考虑。任务节点规模实验表明，较高覆盖率并不必然对应较低队列负载。当覆盖能力提升后，进入 UAV 队列的数据量也会增加；若 Sink 回传能力或 UAV 间转发能力未同步增强，系统仍可能出现队列积压。后续可引入更精细的队列状态反馈、链路拥塞感知机制或回传优先调度策略，使算法在提高覆盖和吞吐的同时进一步增强长期队列稳定性。
...
总体来看，本文提出的 CSA-L-SHADE 算法在当前仿真设置下改善了异构 UAV 集群动态路由规划中的覆盖保持能力和综合优化性能。更真实的通信环境、显式路由机制、更充分的统计验证、队列感知搜索引导以及自适应参数控制，仍是后续需要继续推进的方向。

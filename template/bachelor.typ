...
表 5-2 给出了三种算法在固定环境下的主评价指标结果。

#align(center)[表 5-2 固定环境下三种算法主评价指标结果]
...
为分析综合目标值改善的来源，表 5-3 给出了三种算法在吞吐量、队列负载和能耗方面的结果。

#align(center)[表 5-3 固定环境下三种算法通信性能与资源消耗指标结果]
...
图 5-1 至图 5-3 展示了三种算法在最终 $J_"total"$、coverage ratio 和 $V_"cov"^"norm"$ 上的分布情况。L-SHADE 的最终目标值较高、覆盖率较低、覆盖违约较大；CFI-L-SHADE 在三项指标上均优于 L-SHADE；CSA-L-SHADE 取得最低的平均 $J_"total"$、最高的 coverage ratio 和最低的 $V_"cov"^"norm"$。该分布结果与表 5-2 一致，说明这次改进不是个别运行中的偶然结果。

...
#show: nwpu-thesis.with(
  ...
  abstract: (
    content: [
      灾后应急通信首先面临基础设施失效。地面基站、传输链路和供电设施可能同时受损。通信网络必须快速重组。异构无人机集群部署灵活，覆盖位置可调整。其空中中继能力较强，适合构建临时通信网络。但在这类网络中，路由规划并不等同于路径选择。飞行轨迹会影响覆盖范围和链路质量。链路速率又会改变缓存队列和能量消耗。多因素交织后，问题具有高维、非线性、非光滑和强时序耦合特征。原始 L-SHADE 用于该场景时，随机初始化难以保证覆盖质量。其参数控制也缺少覆盖状态感知。本文围绕这些不足，研究异构无人机集群通信网络动态路由规划方法。
      ...
      在算法设计上，本文提出覆盖状态感知 L-SHADE 算法，即 CSA-L-SHADE。算法以 L-SHADE 为基础连续优化框架，将 UAV 在各控制段内的期望速度、期望航向角和发射功率编码为高维连续控制向量。为缓解随机初始化难以形成覆盖结构的问题，本文设计覆盖优先初始化机制，根据地面任务节点空间分布生成覆盖导向初始个体，改善初始搜索入口。随后，算法利用精英覆盖违约均值、coverage-safe 个体比例和覆盖改善趋势刻画种群覆盖状态，将搜索过程划分为覆盖构造、覆盖稳定、性能精修和覆盖恢复四类状态，并据此动态调节 $p$、$F$ 和 $"CR"$。

      本文设置固定的异构仿真环境，对 L-SHADE、CFI-L-SHADE 和 CSA-L-SHADE 开展对比实验。CSA-L-SHADE 在多项指标上占优，与原始 L-SHADE 相比，CSA-L-SHADE 平均覆盖率提高约 8.09%。归一化覆盖违约降低约 37.64%。归一化吞吐量提高约 64.25%，归一化队列负载降低约 19.64%。当前仿真结果表明，覆盖优先初始化可改善初始种群覆盖不足。覆盖状态感知参数控制则加强后期覆盖保持，并带动综合目标继续下降。
    ],
    keywords: ("异构无人机集群", "动态路由规划", "覆盖状态感知", "L-SHADE", "应急通信"),
  ),
  abstract-en: (
    content: [
      Post-disaster emergency communication first faces infrastructure failure. Ground base stations, transmission links and power supply facilities may be damaged at the same time. The communication network must be quickly reorganized. The deployment of heterogeneous UAV clusters is flexible, and the coverage position can be adjusted. It has strong air relay ability and is suitable for building temporary communication networks. But in this kind of network, routing planning is not equivalent to path selection. The flight trajectory will affect the coverage and link quality. The link rate will change the cache queue and energy consumption. After multi-factor intertwining, the problem has the characteristics of high-dimensional, non-linear, non-smooth and strong timing coupling. When the original L-SHADE is used in this scene, random initialization is difficult to ensure the coverage quality. Its parameter control also lacks coverage state perception. Focusing on these shortcomings, this paper studies the dynamic routing planning method of heterogeneous UAV cluster communication network.  
      ...
      In terms of algorithm design, this paper proposes a coverage-state-aware L-SHADE algorithm, named CSA-L-SHADE. Based on L-SHADE as a continuous optimization framework, the algorithm encodes UAVs’ expected speed, expected heading angle, and transmission power in each control segment as high-dimensional continuous control vectors. To alleviate the problem that random initialization might not form a coverage structure, a coverage-priority initialization mechanism is designed, generating coverage-guided initial individuals according to the spatial distribution of ground mission nodes to improve the initial search entry. Subsequently, the algorithm uses elite coverage violation mean, the proportion of coverage-safe individuals, and coverage improvement trends to characterize the population’s coverage state, dividing the search process into four states: coverage construction, coverage stabilization, performance fine-tuning, and coverage recovery, and dynamically adjusts $p$, $F$, and $"CR"$ accordingly.

      This paper sets up a fixed heterogeneous simulation environment and conducts comparative experiments on L-SHADE, CFI-L-SHADE and CSA-L-SHADE. CSA-L-SHADE dominates in many indicators. Compared with the original L-SHADE, the average coverage of CSA-L-SHADE is about 8.09% higher. The default of normalized coverage decreased by about 37.64%. The normalized throughput is increased by about 64.25%, and the normalized queue load is reduced by about 19.64%. The current simulation results show that the initialization of coverage priority can improve the insufficient coverage of the initial population. The control of coverage state perception parameters strengthens the maintenance of later coverage and drives the comprehensive target to continue to decline.
    ],
...
== 研究背景与意义
无人机（Unmanned Aerial Vehicle, UAV）具有空中巡航的能力，可以在灾情发生后快速进入受灾地区，对地面情况进行全方位、全覆盖的监测，从而为救援工作提供重要的实时数据。UAV还可以搭载各类传感器和摄像设备，获取高清晰度、多角度的图像和视频信息，为灾情的详细勘察提供可靠的技术支持。除此之外，UAV还可以根据预设的路径和航线，进行自主飞行和巡航，无需人工操控，降低人力成本，提高灾情监测的效率。因此，UAV 常用于灾后应急通信@HBGL202514005。然而，当地震、洪涝或山火破坏地面通信设施时，UAV应用场景的难点不止覆盖可行性。灾后初期，任务节点数据常呈突发到达状态。数据流又具有持续性。若 UAV 网络覆盖不及时，回传链路也未成形，缓存队列会迅速积压。随后，系统会出现时延升高、缓存紧张和数据丢失。因此，UAV 通信网络路由规划需同时考虑覆盖建立、队列演化和持续向 Sink 回传。

随着应用场景变复杂，UAV 通信网络已不再只是航迹规划或静态部署问题，而是轨迹控制、任务覆盖、链路传输、队列演化和能量消耗交织在一起的动态优化问题@2024Evolutionary。UAV 的速度、航向角和位置变化决定其与地面任务节点、其他 UAV 及 Sink 节点之间的空间距离，并影响覆盖关系、信道增益和链路传输速率；覆盖状态决定地面数据能否进入 UAV 队列，队列能否及时泄放又依赖于 UAV–UAV 链路和 UAV–Sink 链路的回传能力。高速飞行和高功率发射还会加快能量消耗，剩余能量反过来限制后续运动和通信。轨迹、覆盖、队列、回传和能耗之间不是简单叠加关系，单独优化其中一个环节，可能会把压力转移到另一个环节@2024Evolutionary。在多 UAV 协同场景下，异构性会让问题更难处理。不同 UAV 在最大速度、覆盖半径和初始能量等方面存在差异，路由规划需要在有限资源条件下协调运动控制、覆盖服务、数据回传和能量消耗，统一控制策略或割裂式优化方法很难满足这一要求@9417264。

该问题在优化求解层面较为复杂。其难点主要来自高维变量、非线性关系、非光滑变化和强时序耦合。在时序演化过程中，当前控制量不会只影响当前收益。它会改变 UAV 位置、缓存队列和剩余能量，并继续作用于后续状态。覆盖关系具有阈值特征，距离变化可能引发离散切换。通信速率则同时受信道衰减和发射功率约束。传统精确优化方法难以直接适配该问题。其建模过程较繁，求解成本较高，面对更大规模网络时扩展能力不足@2025UAV。因此，本文不追求直接精确求解，而采用启发式或元启发式算法获得近似解@Das2026@AGRAWAL20241。

差分进化及其改进算法适合处理连续变量、高维和复杂约束优化问题。L-SHADE@LI2022350 在 SHADE@6900380 成功历史参数记忆机制基础上加入线性种群规模缩减策略，可在搜索前期保留探索能力，在后期提高收敛效率，因而适合作为 UAV 控制序列优化的基础框架。不过，原始 L-SHADE 仍是通用连续优化算法，参数自适应主要依赖历史成功经验，并不直接利用 UAV 通信网络中的覆盖状态。在动态覆盖路由问题中，覆盖状态既是目标函数中的惩罚项，也影响地面数据到达、队列演化和后续通信性能。本文据此在 L-SHADE 中加入 UAV 覆盖结构信息，设计面向覆盖状态的改进机制。

围绕上述问题，本文研究异构无人机集群通信网络动态路由规划。模型部分同时考虑 UAV 运动约束、通信能力、地面节点覆盖、队列演化和能量消耗；算法部分提出覆盖状态感知 L-SHADE，即 CSA-L-SHADE（Coverage-State-Aware L-SHADE）。该算法以 L-SHADE 为基础框架，先用覆盖优先初始化改善初始搜索入口，再利用覆盖状态感知参数控制机制调节搜索行为，以提高 UAV 动态覆盖路由中的覆盖保持能力和综合优化性能。该方法可在有限计算预算内生成 UAV 控制序列。控制序列可直接用于仿真执行。相关结果可服务灾后应急通信、临时网络部署和无人机协同中继分析。

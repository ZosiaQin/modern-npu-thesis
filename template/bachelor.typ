...
#show: nwpu-thesis.with(
  ...
  abstract-en: (
    content: [
      ...
      The algorithm design starts from the two shortcomings of the original L-SHADE: random initialization does not use the spatial information of task nodes, and the early search is easy to cover insufficient. Parameter adaptation mainly relies on historical successful experience, lacks explicit perception of UAV coverage status, and may turn to performance optimization prematurely when the coverage is not stable. To this end, this paper proposes a covering state-aware L-SHADE algorithm, namely CSA-L-SHADE. The algorithm retained the continuous optimization framework of L-SHADE, introduced cover-oriented individuals in the initialization phase, and constrained parameter changes with population coverage status in the iteration phase.

      In the fixed heterogeneous simulation environment, the comparison results of L-SHADE, CFI-L-SHADE and CSA-L-SHADE are significantly different. Compared with the original L-SHADE, the average coverage rate of CSA-L-SHADE is increased by about 8.09%, the normalized coverage violation is reduced by about 37.64%, the throughput is increased by about 64.25%, and the queue load is reduced by about 19.64%. From the perspective of the convergence process, the coverage priority initialization mainly alleviates the insufficient coverage of the initial population, and the coverage state aware parameter control reduces the damage of the existing coverage structure by differential disturbance in the middle and late stages.
...
在参数生成上，L-SHADE 仍从成功历史记忆中采样 $F_i$ 和 $C R_i$；在种群规模上，它按线性缩减规则从较大的初始种群逐步过渡到较小种群。本文提出的 CSA 机制并不替换这些步骤。它只在参数采样之后读取种群覆盖状态，再对 $p$、$F$ 和 $C R$ 做状态相关约束。换句话说，CSA-L-SHADE 的连续搜索主体仍是 L-SHADE，新增部分负责告诉算法当前应更重视覆盖构造、覆盖保持还是性能细化。
...
随机初始化容易把 UAV 带向与任务节点无关的区域，搜索一开始就可能处在低覆盖状态。为降低这种冷启动代价，CSA-L-SHADE 在初始种群中加入覆盖优先初始化机制（Coverage-First Initialization, CFI）。CFI 利用地面任务节点分布生成一部分覆盖导向个体，但它只改变初始入口，不干预后续变异、交叉、选择、外部档案和成功历史记忆更新。

生成 CFI 个体时，先从任务节点中抽取若干覆盖引导点。记任务节点集合为 $cal(K) = brace.l 1, 2, dots, K brace.r$，节点 $k$ 的位置为 $bold(q)_k$，引导点数量取
...
得到扰动目标点以后，CFI 开始为每架 UAV 生成分段控制序列。UAV $i$ 在控制段 $g in cal(G)$ 的控制向量仍写作
...
吞吐量结果中，原始 L-SHADE 的 $R_"norm"$ 为 $0.2826$，CFI-L-SHADE 为 $0.4368$，CSA-L-SHADE 为 $0.4641$。相较原始 L-SHADE，CSA-L-SHADE 的提升约为 64.2%。这个差距不应简单归因于发射功率变大，因为能耗并没有同步升高。更合理的解释是，覆盖结构稳定后，任务数据能更持续地进入 UAV 队列；同时部分 UAV 的位置更利于回传，Sink 侧接收到的数据随之增加。
...
地面任务节点数量变化时，覆盖压力和数据到达量都会改变。为观察这种负载变化带来的影响，本节调整任务节点规模，并比较三种算法的覆盖率、能耗、队列负载和吞吐量。这里重点看趋势变化，所以使用三种算法的对比折线图，不再展开 CSA-L-SHADE 的参数敏感性。
...
图 5-5 中，原始 L-SHADE 在多数场景下能耗更高，波动也更明显。缺少任务结构引导时，种群可能通过额外移动或低效通信弥补覆盖和回传不足。CFI-L-SHADE 与 CSA-L-SHADE 的能耗整体较低，说明覆盖导向信息减少了一部分无效搜索。需要补充的是，CSA-L-SHADE 并非每个任务规模下都最低能耗；它的优势更适合结合覆盖率和吞吐量一起判断。
...
CSA-L-SHADE 的变化发生在迭代过程中。固定环境实验里，它的最终覆盖率为 $0.8943$，覆盖违约度为 $0.1076$，吞吐量达到 $0.4641$，队列负载和能耗分别为 $0.2839$ 与 $0.000391$。单看任意一个指标，都不足以说明机制是否有效；把它们合在一起看，更接近本文想验证的问题：覆盖结构被保留下来以后，回传能力和队列状态是否也能随之改善。结果显示，吞吐量提高并没有伴随能耗增加，因此该改进更像是调整了轨迹、覆盖与回传关系，而不是简单用更多资源换取性能。

CSA-L-SHADE 的结果也需要留出边界。从标准差和收敛曲线阴影范围看，部分指标仍然有波动，尤其是吞吐量和后期目标函数值。覆盖状态感知提高了搜索上限，但随机种子和场景结构的影响并没有消失。后续实验至少应增加独立运行次数，并在更多任务节点分布、UAV 规模和数据产生强度下重复验证。
...
实验部分比较了 L-SHADE、CFI-L-SHADE 和 CSA-L-SHADE，并进一步改变任务节点数量观察趋势。固定环境中，CSA-L-SHADE 的覆盖率为 $0.8943$，覆盖违约度为 $0.1076$，吞吐量为 $0.4641$，队列负载和能耗分别为 $0.2839$、$0.000391$。收敛曲线更值得注意的是中后期差异：CSA-L-SHADE 并非只依赖更好的初始下降，而是在覆盖结构形成后仍能继续修正控制序列。任务节点规模实验也显示，它在多数规模下保持了较高覆盖率，并在中高负载场景下维持较好的吞吐水平。

== 不足与展望

本文模型和实验仍有简化。这样做便于单独观察覆盖状态感知机制，但结论外推时需要谨慎。后续工作可从以下方向展开。

通信信道模型还比较简化。当前模型采用二维距离平方衰减，没有显式加入三维距离、高度变化、地形遮挡、LoS/NLoS 概率切换、多径衰落和同频干扰。灾后场景中，建筑物遮挡、地形起伏和临时干扰源都可能改变链路速率；换成更真实的空地信道后，CSA-L-SHADE 对覆盖与回传结构的调节能力还需要重新检验。
...
在当前仿真设置下，CSA-L-SHADE 改善了覆盖保持和综合优化表现，但这一结论还不能直接推广到所有应急通信场景。后续需要进一步检验更真实的通信环境、显式路由机制、统计显著性、队列感知搜索引导以及自适应参数控制。

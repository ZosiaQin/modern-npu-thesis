...
#show: nwpu-thesis.with(
  ...
  abstract: (
    content: [
      ...
      算法设计则从原始 L-SHADE 的两个短板切入：随机初始化没有使用任务节点空间信息，搜索早期容易覆盖不足；参数自适应主要依赖历史成功经验，缺少对 UAV 覆盖状态的显式感知，覆盖尚未稳定时可能过早转向性能优化。为此，本文提出覆盖状态感知 L-SHADE 算法，即 CSA-L-SHADE。该算法保留 L-SHADE 的连续优化框架，在初始化阶段引入覆盖导向个体，在迭代阶段用种群覆盖状态约束参数变化。

      固定异构仿真环境下，L-SHADE、CFI-L-SHADE 和 CSA-L-SHADE 的对比结果差异较明显。相较原始 L-SHADE，CSA-L-SHADE 的平均覆盖率提高约 8.09%，归一化覆盖违约降低约 37.64%，吞吐量提高约 64.25%，队列负载降低约 19.64%。从收敛过程看，覆盖优先初始化主要缓解初始种群覆盖不足，覆盖状态感知参数控制则在中后期减少已有覆盖结构被差分扰动破坏的情况。
    ],
    keywords: ("异构无人机集群", "动态路由规划", "覆盖状态感知", "L-SHADE", "应急通信"),
  ),
  abstract-en: (
    content: [
      In a heterogeneous UAV swarm communication network, UAVs with different capabilities jointly undertake coverage, forwarding, and data aggregation tasks. Unlike a homogeneous swarm, the UAVs may differ in maximum speed, coverage radius, communication bandwidth, transmission power, cache capacity, and energy capacity, so their roles in the same mission are not identical. During operation, UAV-ground links provide task-area coverage, UAV-UAV links support aerial forwarding, and UAV-Sink links deliver aggregated data to the sink. Such networks are mainly used in infrastructure-limited scenarios, including ad hoc communication, regional monitoring, and post-disaster emergency communication.

      Dynamic routing planning for heterogeneous UAV swarms is not a simple path selection problem. UAV trajectories change both ground coverage and link quality; link rates then affect the release of cached data, while high-speed flight and high-power transmission consume more energy. These factors propagate along the time horizon, making the problem high-dimensional, nonlinear, non-smooth, and strongly time-coupled. For this type of continuous optimization problem, a generic metaheuristic algorithm may fail to form stable coverage and return links unless the task structure is used explicitly.

      This thesis focuses on a common post-disaster setting: ground communication infrastructure is damaged, task nodes keep generating data, and the network must be reorganized quickly. In this setting, routing planning cannot be reduced to fixed path selection or a single next-hop decision. UAV motion control, coverage relationships, link rates, and queue states have to be modeled together. Therefore, UAV speed, heading angle, and transmission power are treated as joint decision variables so that the swarm can maintain ground coverage and data return during movement. In this sense, routing planning in this thesis is a dynamic optimization problem over heterogeneous UAV control sequences, where coverage and return capability are prioritized before throughput, queue load, and energy consumption are further optimized.

      The model is built from a fixed-altitude two-dimensional emergency communication scenario. The system contains multiple heterogeneous UAVs, several ground task nodes, and a fixed Sink node. After a task node is covered by a UAV, data arrivals are generated and then forwarded through UAV-UAV and UAV-Sink links. Around this process, UAV motion constraints, wireless link rates, coverage relationships, queue evolution, functional return capability, and energy consumption are included in one dynamic model. A composite objective function is then constructed from normalized throughput, queue load, energy consumption, coverage violation, and connectivity violation.

      The algorithm starts from two weaknesses of the original L-SHADE. Random initialization does not use the spatial information of task nodes, so early search may suffer from poor coverage. Its parameter adaptation mainly follows historical successful trials and has no explicit awareness of the current UAV coverage status, which may push the search toward performance refinement before coverage is stable. To address these issues, a coverage status-aware L-SHADE algorithm, named CSA-L-SHADE, is proposed. It keeps the continuous optimization framework of L-SHADE, adds coverage-oriented individuals during initialization, and constrains parameter changes during iteration according to the population coverage status.

      L-SHADE, CFI-L-SHADE, and CSA-L-SHADE are compared in a fixed heterogeneous simulation environment. Compared with the original L-SHADE, CSA-L-SHADE improves the average coverage ratio by about 8.09%, reduces the normalized coverage violation by about 37.64%, increases throughput by about 64.25%, and reduces queue load by about 19.64%. The convergence process suggests that coverage-first initialization mainly alleviates insufficient initial coverage, while coverage status-aware parameter control helps protect existing coverage structures from being disrupted by differential perturbations in the middle and late search stages.
    ],
  ...
  design-summary: [
    本文围绕异构无人机集群通信网络动态路由规划问题，建立了包含 UAV 运动控制、无线通信、地面任务节点覆盖、数据队列、功能性回传能力和能量消耗的动态优化模型，并在 L-SHADE 基础上提出 CSA-L-SHADE 算法。覆盖优先初始化用于缓解随机初始化带来的低覆盖冷启动问题，覆盖状态感知参数控制则根据种群覆盖状态调整后续搜索行为。固定环境实验中，CSA-L-SHADE 相较原始 L-SHADE 和 CFI-L-SHADE 在综合目标值、覆盖率、覆盖违约、吞吐量、队列负载和能耗等指标上表现更稳。对异构 UAV 动态覆盖路由问题而言，把覆盖状态显式纳入搜索过程，比单纯依赖通用参数自适应更贴近任务结构。
...
第 3 章已经把路由规划写成控制序列优化问题，本章转向求解算法。原始 L-SHADE 可以处理连续控制变量，但它不知道当前种群是否已经形成有效覆盖结构，也无法区分覆盖构造、覆盖保持和性能精修这些阶段。因此，本章先给出 L-SHADE 在本文问题中的编码、解码、评估和选择方式，再引入覆盖优先初始化与覆盖状态感知参数控制，最后整理 CSA-L-SHADE 的流程和复杂度。
...
本章用仿真实验检验 CSA-L-SHADE 在异构无人机集群动态路由规划中的表现。实验在统一固定环境下比较 L-SHADE、CFI-L-SHADE 和 CSA-L-SHADE，并观察综合目标、覆盖性能、覆盖违约、连通违约、吞吐量、队列负载、能耗以及收敛过程的变化。这里关心的不只是最终数值谁更优，还包括覆盖状态感知机制是否真的改变了搜索后期的行为。
...
把五个指标放在一起看，CSA-L-SHADE 在固定环境下形成了一个较稳的折中：覆盖率和吞吐量提高，覆盖违约度、队列负载和能耗降低。尤其是吞吐量增加而能耗没有上升，这一点排除了“靠额外能耗换吞吐”的简单解释，更像是覆盖关系和回传结构被重新协调后的结果。
...
图 5-5 中，原始 L-SHADE 在多数场景下能耗偏高，波动也更大。这与其缺少任务结构引导有关：当种群无法稳定形成覆盖与回传结构时，UAV 可能通过更多无效移动或低效通信来弥补性能不足。CFI-L-SHADE 和 CSA-L-SHADE 的能耗整体低于原始 L-SHADE，覆盖结构引导在一定程度上减少了无效搜索。需要说明的是，CSA-L-SHADE 并非在每个任务规模下都是能耗最低，但它通常同时保持较高覆盖率和吞吐量，因此更适合从综合指标而不是单一能耗指标来评价。
...
从图 5-7 来看，CSA-L-SHADE 在中高任务节点规模下的吞吐水平更高。原因不只是服务范围扩大，还包括回传结构的调整。覆盖状态感知机制帮助算法保留较好的覆盖个体，同时也让搜索过程继续寻找更有利于 Sink 汇聚的数据转发结构。原始 L-SHADE 在多数规模下吞吐量较低；缺少问题结构引导时，通用差分搜索很难充分利用 UAV 的通信资源。
...
实验部分对 L-SHADE、CFI-L-SHADE 和 CSA-L-SHADE 进行了固定异构环境对比，并考察了不同任务节点数量下的变化。固定环境实验中，CSA-L-SHADE 最终覆盖率达到 $0.8943$，覆盖违约度降至 $0.1076$，吞吐量提升至 $0.4641$，队列负载和能耗分别降至 $0.2839$ 和 $0.000391$。收敛曲线中更值得注意的是中后期差异：CSA-L-SHADE 不是只在初始阶段下降更快，而是在覆盖结构基本形成后仍能继续修正控制序列。任务节点规模实验也显示，它在多数规模下能保持较高覆盖率，并在中高负载场景下维持较好的吞吐水平。

== 不足与展望

本文模型和实验仍然建立在若干简化条件上。这些设定便于单独观察覆盖状态感知机制的作用，但也限制了结论的外推范围。后续工作主要有以下几个方向。

通信信道模型仍较为简化。本文采用基于二维距离平方衰减的信道模型，未显式考虑三维距离、高度变化、复杂地形遮挡、LoS/NLoS 概率切换、多径衰落和同频干扰等因素。灾后场景中，建筑物遮挡、地形起伏和临时干扰源都可能改变链路速率；如果换成更真实的空地信道模型，CSA-L-SHADE 对覆盖与回传结构的调节能力还需要重新检验。
...
另外，CSA-L-SHADE 的状态控制参数仍依赖人工设定。本文根据覆盖状态设计了参数约束范围，但不同任务规模、UAV 异构程度和数据到达强度下，合适阈值可能并不相同。后续可以尝试自适应阈值调整、强化学习辅助参数控制或多状态记忆机制，使算法在不同环境中自动修正状态判定条件和参数范围。

在当前仿真设置下，CSA-L-SHADE 改善了异构 UAV 集群动态路由规划中的覆盖保持和综合优化表现，但它还不能直接推广到所有应急通信场景。下一步最需要检验的，是更真实的通信环境、显式路由机制、更充分的统计验证、队列感知搜索引导以及自适应参数控制。
